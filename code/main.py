...
def train3d(args):
    ...
    root_dir = f'{os.environ["INPUT_PATH"]}/cv{args.cv}'
    print(f'Input root dir: {root_dir}')
    print('Train dir:', f'{root_dir}/train')
    ...
    # train_dataset = NCCT111Dataset(
    #                     os.environ["INPUT_PATH"]+'train', mode='train',
    #                     no_noisy=args.no_noisy,
    #                     flip_image=model.compare_left_right,
    #                     curriculum=args.curriculum)
    # val_dataset = NCCT111Dataset(os.environ["INPUT_PATH"]+'val', mode='val', flip_image=model.compare_left_right)
    ...
    for epoch in epoch_pbar:   #to do
        ...
        if args.curriculum:
            ...
            for i in range(len(cur_transform)):
                if isinstance(cur_transform[i], SampleProbabilityMap):
                    t = math.exp(-8*(1-epoch/args.num_epochs)**2)  # need global max epochs
        ...
        #Verify the performance og the global model
        ...
        #Save info that the derver needs to know
        output_dict = {}
        output_dict["metadata"] = {
            'epoch':epoch, 'datasetSize': len(train_dataset), 'importance': 1.0}
        output_dict["metrics"] = {
            'val_precision': measure_val['ppv'],
            'basic/confusion_tp': -1,
            'basic/confusion_fp': -1,
            'basic/confusion_fn': -1,
            'basic/confusion_tn': -1,
            'val_dsc': measure_val['dsc'],
            'val_loss': measure_val['loss']}
        ...
        torch.save(weights, os.environ["LOCAL_MODEL_PATH"])
        # choose one?
...
if __name__ == '__main__':
    ...
    # for model
    parser.add_argument('--model', type=str, required=False, default='MGAPDAFAPUNet')
